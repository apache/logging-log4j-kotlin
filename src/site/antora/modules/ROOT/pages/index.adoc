...
Since `log4j-core` *also* depends transitively on `log4j-api`, this may result in Maven downgrading the `-api` artifact which can https://github.com/apache/logging-log4j-kotlin/issues/144#issuecomment-4090207780[cause issues].
We recommend that projects https://logging.apache.org/log4j/2.x/manual/installation.html[leverage the `log4j-bom`] as well.
